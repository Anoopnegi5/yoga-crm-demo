...
import { X, CreditCard, Smartphone, Copy, Check, ExternalLink, Loader2, CheckCircle2, AlertCircle, IndianRupee, Share2 } from 'lucide-react';
...
type ModalStep = 'choose' | 'collecting' | 'generating_link' | 'link_ready' | 'success' | 'error';
...
export const PaymentCheckoutModal: React.FC<PaymentCheckoutModalProps> = ({
  ...
  // Collect payment directly via Razorpay checkout (trainer side)
  ...
  // Generate WhatsApp payment link (client pays themselves)
  const handleGenerateLink = async () => {
    ...
    if (result) {
      ...
      setErrorMsg('Payment link generate karne mein error. Keys check karein.');
      setStep('error');
    }
  };
      ...
      <div className="bg-white rounded-3xl w-full max-w-md shadow-2xl border border-slate-200 overflow-hidden">
        ...
        <div className="p-5 bg-gradient-to-r from-emerald-700 to-teal-700 text-white flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="font-extrabold text-base">Collect Payment</h3>
              <p className="text-[11px] text-emerald-200">{clientName} — ₹{editAmount.toLocaleString()}</p>
            </div>
          </div>
        ...
        <div className="p-6">
            ...
            <div className="space-y-4">
              <div className="p-3 rounded-2xl bg-slate-50 border border-slate-100 space-y-1">
                ...
                <div className="flex items-center gap-1.5 pt-1">
                  ...
                  <input
                    ...
                    min={1}
                  />
                  <span className="text-xs text-slate-400 font-medium">(edit if needed)</span>
                </div>
              </div>

              <p className="text-xs font-bold text-slate-500 text-center uppercase tracking-wider">Choose Payment Method</p>
              ...
              >
                ...
                <div className="text-left">
                  ...
                  <p className="text-[11px] text-emerald-200 font-medium">Open payment page on this device — UPI, Card, Net Banking</p>
                </div>
              </button>
          ...
          {step === 'collecting' && (
            <div className="py-10 text-center space-y-4">
              ...
              <p className="font-extrabold text-slate-700">Opening payment gateway...</p>
              <p className="text-xs text-slate-400">Razorpay checkout window should appear shortly</p>
            </div>
          )}

          {/* STEP: Generating link */}
          ...
          {step === 'success' && (
            <div className="py-8 text-center space-y-4">
              ...
              <div>
                <p className="font-extrabold text-xl text-emerald-700">Payment Successful! 🎉</p>
                <p className="text-sm text-slate-500 mt-1">₹{editAmount.toLocaleString()} collected from {clientName}</p>
              </div>
              ...
              <button
                onClick={handleClose}
                className="w-full py-3 rounded-2xl bg-emerald-600 text-white font-extrabold text-sm hover:bg-emerald-700 transition-all"
              >
                Done ✓
              </button>
            </div>
          ...
          {step === 'error' && (
            <div className="py-8 text-center space-y-4">
              ...
              <div>
                <p className="font-extrabold text-lg text-rose-700">Payment Failed</p>
