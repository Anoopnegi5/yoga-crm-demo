...
export const ClientRegistrationWizard: React.FC = () => {
  const { addClient, clients, showSuccessToast } = useApp();

  const [step, setStep] = useState<number>(1);
  const [submitted, setSubmitted] = useState(false);

  // Dynamic Group Batches strictly created by trainer / existing in active clients
  ...
  const availableBatches = existingGroupNames.length > 0 
    ? existingGroupNames 
    : ['Personal One-on-One Session', 'Morning Vinyasa Batch', 'General Yoga Batch'];
      ...
      <div className="bg-white rounded-3xl w-full max-w-2xl shadow-2xl border border-slate-100 relative overflow-hidden">
        ...
        {submitted ? (
          <div className="p-8 sm:p-12 text-center space-y-7 animate-fadeIn">
            <div className="w-20 h-20 rounded-3xl bg-gradient-to-tr from-emerald-500 to-teal-600 text-white flex items-center justify-center mx-auto text-3xl shadow-xl shadow-emerald-950/20 ring-4 ring-emerald-100">
              ✨
            </div>
              ...
              <div className="space-y-5 animate-fadeIn">
                ...
                <div className="bg-purple-50/60 p-4 rounded-3xl border border-purple-100 space-y-3">
                  ...
                  <select
                    ...
                    onChange={(e) => setSelectedBatchDropdown(e.target.value)}
                    className="w-full px-4 py-3 rounded-2xl bg-white border border-purple-200 text-xs font-bold text-purple-900 focus:ring-2 focus:ring-purple-500/20 outline-none shadow-sm"
                  >
