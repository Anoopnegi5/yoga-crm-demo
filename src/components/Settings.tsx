...
export const Settings: React.FC<SettingsProps> = ({ onLogout }) => {
  const { trainerProfile, updateTrainerProfile, exportBackupData, importBackupData, websiteCMS, updateWebsiteCMS } = useApp();
    ...
    <div className="space-y-6 animate-fadeIn max-w-4xl mx-auto">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white p-6 sm:p-7 rounded-3xl shadow-soft border border-slate-100 space-y-5">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-2xl bg-emerald-100 text-emerald-800 font-black text-xl flex items-center justify-center shadow-sm">
                💾
              </div>
              <div>
                <h4 className="font-serif font-extrabold text-slate-900 text-base">Studio Data Backup & Restore</h4>
                <p className="text-[11px] text-slate-500 font-medium">Export all records to JSON or restore a previous backup</p>
              </div>
            </div>

            <p className="text-xs text-slate-600 leading-relaxed font-medium">
              Download a complete JSON backup of all client profiles, attendance history, and fee records or restore from a file.
            </p>
            ...
            <div className="space-y-3 pt-1">
              <button
                ...
                className="w-full py-3.5 rounded-2xl bg-emerald-600 hover:bg-emerald-700 text-white font-extrabold text-xs shadow-md transition-all flex items-center justify-center gap-2 hover:scale-[1.02] active:scale-[0.98]"
              >
                <Download className="w-4 h-4 text-white" />
                <span>Download Backup File</span>
              </button>

              <label className="w-full py-3.5 rounded-2xl bg-slate-100 hover:bg-slate-200 border border-slate-200 text-slate-800 font-extrabold text-xs transition-all flex items-center justify-center gap-2 cursor-pointer hover:scale-[1.02] active:scale-[0.98]">
